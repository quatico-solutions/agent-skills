...
/**
 * Cursor plugin validation script.
 * Adapted from https://github.com/cursor/plugin-template/blob/main/scripts/validate-template.mjs
 *
 * Validates:
 * - .cursor-plugin/marketplace.json structure
 * - Plugin plugin.json in each listed plugin directory
 * - Frontmatter on skills (name + description), commands (name + description),
 *   rules (description), agents (name + description)
 * - Path references resolve to existing files/directories
 * - Names are lowercase kebab-case
 * - Marketplace entry names match plugin.json names
 */
