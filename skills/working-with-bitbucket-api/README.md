...
- **Bundled in qs-config**: `bb` script ships in `bin/bb` within this skill directory; `install-dependencies.sh` copies it to `$(brew --prefix)/bin/` — a copy, not a symlink (plugin cache dirs are version-based and vanish on update, dangling any symlink into them), and not `~/bin` (not on macOS default PATH). Staleness of the copy is handled by the SKILL.md Step 0 version gate, which re-runs the installer on version drift.
