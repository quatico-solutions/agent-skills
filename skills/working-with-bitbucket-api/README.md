...
- **Bundled in qs-config**: the CLI lives at `cli/bb` in this repository, outside the skill directory, and Homebrew installs it from `quatico-solutions/tap/bb`. Staleness is handled by the SKILL.md Step 0 version gate, which upgrades the formula on version drift.
