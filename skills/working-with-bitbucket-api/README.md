...
- **Installed via Homebrew**: the CLI lives at `cli/bb` in this repository, outside the skill directory. Homebrew installs and upgrades it from `quatico-solutions/tap/bb`; the formula is bumped when a release publishes a new `bb` version. The SKILL.md Step 0 gate checks that PATH's `bb` actually came from Homebrew and that it is current.
